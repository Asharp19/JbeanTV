...
interface CoinLeaderboardTableProps {
  coinpair: string;
  currentPair: {
    symbol: string;
    name: string;
...
export function CoinLeaderboardTable({
  coinpair,
  currentPair,
}: CoinLeaderboardTableProps) {
  const { leaderboardEntries, loading, error, usingFallbackData } =
    useLeaderboard();
  const { addPair } = useTickerContext();
  ...
  // Add the coinpair to the watched list for real-time price updates
  useEffect(() => {
  ...
  // Filter entries for this specific coin
  const filteredEntries = useMemo(() => {
    // Make the pair comparison case-insensitive
    const normalizedCoinpair = coinpair.toUpperCase();

    return leaderboardEntries
      .filter((entry) => entry.pair.toUpperCase() === normalizedCoinpair)
      .map((entry, index) => ({ ...entry, rank: index + 1 })); // Recalculate ranks
  }, [leaderboardEntries, coinpair]);
      ...
      <Card className="overflow-hidden bg-background-secondary/40 border-primary backdrop-blur-xl shadow-glass p-6">
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <p className="mt-4 text-slate-400">
            Loading {currentPair.name} leaderboard data...
          </p>
        </div>
      </Card>
    );
      ...
      <Card className="overflow-hidden bg-background-secondary/40 border-primary backdrop-blur-xl shadow-glass p-6">
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <p className="text-red-500 font-medium">
            Error loading {currentPair.name} leaderboard data
          </p>
      ...
      <Card className="overflow-hidden bg-background-secondary/40 border-primary backdrop-blur-xl shadow-glass p-6">
        <div className="flex flex-col items-center justify-center py-8">
          <p className="text-slate-400">
            No leaderboard data available for {currentPair.name}
          </p>
        </div>
      </Card>
    );
                      ...
                      <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 p-2 bg-background-secondary/90 backdrop-blur-md text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                        <div className="flex flex-col gap-1">
                          <div className="flex justify-between">
                            <span>Current:</span>
                            <span>${entry.currentPrice.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Predicted:</span>
                            <span>
